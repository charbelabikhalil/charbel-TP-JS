	...
	save() {
  	...
  	displayContactsAsATable(idOfContainer) {
    	...
    	this.listOfContacts.forEach(function(currentContact, index) {
			...
			row.innerHTML = "<td contenteditable=\"true\" onclick=\"editNameCell(this)\" data-contact-id=" + index + ">" + currentContact.name + "</td>"
							+ "<td contenteditable='true' onclick=\"editEmailCell(this)\"data-email-id=" + index + ">" + currentContact.email + "</td>" 
	  ...
	  // Arg 2, On affiche que une copie qui pointe sur les elts de la list "listOfContacts" 
	  displayCopyOfContactsAsATable(idOfContainer, listContacts) {
		// empty the container that contains the results
    	let container = document.querySelector("#" + idOfContainer);
    	container.innerHTML = "";
		...
		// insert Header
		var headerRow = table.insertRow();
		//headerRow.innerHTML = "<th onclick=\"sortNameCopy( + " + listContacts + ")\">" + "Name2" + "</th><th onclick=\"sortMailCopy( " + listContacts + ")\">" + "Mail2" + "</th>";
		headerRow.innerHTML = "<th>" + "Name" + "</th><th>" + "Mail" + "</th>"; 

    	// iterate on the array of users
    	listContacts.forEach(function(currentContact, index) {
        	// creates a row
			var row = table.insertRow();
			...
			trashbin.dataset.contactId = this.listOfContacts.indexOf(currentContact); //index;
			trashbin.style.setProperty("cursor", "pointer"); // ds le css ou <a href ...
			trashbin.className = "trashbinImgs";
